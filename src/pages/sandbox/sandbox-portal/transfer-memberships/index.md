...
Processing Subscription transfers involves actions from the Portal and an API call. The preliminary work, which will result in creating a Membership ID, is done in the Portal. Subscription transfer is done through an API call, which requires the Membership ID previously created in the Portal.
